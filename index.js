...
function customEntries(obj) {
  ...
  const objNew = Object.entries(obj);
  return objNew;
}
...
function customObjectFromEntries(entries) {
  ...
  const numString = entries.map(([key, value]) => {
  ...
  const obj = Object.fromEntries(numString);
  return obj;
}
...
function getObjectValuesSum(obj) {
  ...
  const sum = Object.values(obj).reduce((accum, curNum) => (curNum += accum));
